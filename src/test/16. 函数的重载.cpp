...
void sameNameFunc(int age)
{
    cout << "int type: age is " << age << endl;
    return;
}
void sameNameFunc(string userName)
{
    cout << "string type: username is " << userName << endl;
    return;
}
void sameNameFunc(int age, float heigh)
{
    cout << "age: " << age << " heigh: " << heigh << endl;
    return;
}
void sameNameFunc(float heigh, int age) //顺序不同
{
    cout << "heigh: " << heigh << " age: " << age << endl;
    return;
}
void sameNameFunc(float heigh, int age, string userName) //个数和顺序都不同
{
    cout << "username: " << userName << " heigh: " << heigh << " age: " << age << endl;
    return;
}

// int sameNameFunc(int age)   //该处如果想成功定义，必须是对形参做个数、顺序或者参数类型这三种条件的构造，例如下句
int sameNameFunc(double age)
{
    return age;
}
 ...
 * @return: 
 */
void reloadOfReference(int& arg) //如下调用传入实参如果为10，语义是int& arg = 10,是不通过的，由于引用必须是一个合法的内存空间，即堆区或者栈区数据
{
    cout << "called void reloadOfReference(int& arg)" << endl;
    return;
}
void reloadOfReference(const int& arg) //如下调用传入实参如果为10，语义是const int& arg = 10，该表达式为常量引用，编译器会创建临时栈区空间
{
 ...
 * @return: 
 */
/* void reloadOfReference(int arg)
{
    cout << "called void reloadOfReference(int arg)" << endl;
    return;
} */
 ...
 * @return: 
 */
void displayDemo(int a, int b = 10)
{
    cout << "a = " << a << " b = " << b << endl;
    return;
}
